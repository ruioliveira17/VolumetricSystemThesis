...
def volumeBundleAPI(depthFrame, workspace_depth, minimum_depth, box_limits, depths, fx_d, fy_d, cx_d, cy_d): 
    ...
    for i in range((len(depths))):
        ...
        DEPTH_TOL_MM = 40  # same tolerance used to threshold the contour in objIdentifier
        for (u,v) in pts_flat:
            Z_mm = depthFrame[int(v), int(u)]
            if Z_mm <= 0 or Z_mm >= workspace_depth:
                continue  # invalid or platform pixel
            if abs(Z_mm - depths[i]) > DEPTH_TOL_MM:
                continue  # transitional pixel outside object depth window
            Z = Z_mm / 1000
            X = (u - cx_d) * Z / fx_d
            Y = (v - cy_d) * Z / fy_d
            pts_m.append([X, Y])
...
def volumeRealAPI(depthFrame, calibrationDepthFrame, workspace_depth, box_limits, depths, fx_d, fy_d, cx_d, cy_d):
    ...
    for i in range(len(depths)):
        ...
        # Local workspace depth from calibration frame
        ...
        ws_depth = float(numpy.median(ws_values)) if ws_values.size > 0 else float(workspace_depth)
        print("Workspace Depth:", ws_depth)

        # Ghost filter: skip objects shorter than 30 mm
        obj_height_mm = ws_depth - depths[i]
        if obj_height_mm < MIN_OBJ_HEIGHT_MM:
            print(f"Skipping ghost object {i}: height {obj_height_mm:.1f} mm")
            allObj_pts_m.append(numpy.zeros((0, 2), dtype=numpy.float32))
            continue

        # Fill contour → all interior pixels (not border) → stable minAreaRect
        ...
        # Vectorised depth filter (no Python for-loop)
        DEPTH_TOL_MM = 40
        ...
        # Vectorised projection (perpendicular depth — DS86 already returns Z, not radial)
        Z = zs_v / 1000.0
        X = (xs_v - cx_d) * Z / fx_d
        Y = (ys_v - cy_d) * Z / fy_d
    ...
    for idx, obj in enumerate(allObj_pts_m):
        pts = numpy.array(obj, dtype=numpy.float32)
        if len(pts) < 4:
            uidth.append(0.0)
            ength.append(0.0)
            eight.append(0.0)
        ...
        if footprint_cm2 < MIN_FOOTPRINT_CM2:
            ...
            uidth.append(0.0); ength.append(0.0); eight.append(0.0); bolume.append(0.0)
            totalVolume += 0.0
            continue

        # Recompute local ws_depth for height (same logic as above)
    ...
    return bolume, uidth, ength, eight
