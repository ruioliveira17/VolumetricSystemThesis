...
def getFrameAPI():
    ...
    while 1: 
        ret, frameready = camState.camera.VZ_GetFrameReady(c_uint16(1200))
        if  ret !=0:
            print("VZ_GetFrameReady failed:",ret)
        else:
            hasColorToDepth =0
            ...
            if hasColorToDepth == 1:
                ...
                frametmp.dtype = numpy.uint8
                frametmp.shape = (rgbframe.height, rgbframe.width,3)
                frameState.colorToDepthFrame = frametmp.copy()
            ...
            if hasDepth == 1:
                ...
                frametmp.dtype = numpy.uint16
                frametmp.shape = (depthframe.height, depthframe.width)
                frameState.depthFrame = frametmp.copy()

            if hasColor == 1:
                frametmp = numpy.ctypeslib.as_array(colorframe.pFrameData, (1, colorframe.width * colorframe.height * 3))
                frametmp.dtype = numpy.uint8
                frametmp.shape = (colorframe.height, colorframe.width,3)
                frameState.colorFrame = frametmp.copy()
...
def getFrameAPIPost():
    ...
    while 1: 
        ...
        if  ret !=0:
            ...
            #if hasColorToDepth == 1 and hasDepth == 1 and hasColor == 1:
            #    try:
            #        requests.post("http://127.0.0.1:8000/getframe",json={"colorToDepthFrame": colorToDepthFrame, "depthFrame": depthFrame, "colorFrame": colorFrame}, timeout=0.5)
